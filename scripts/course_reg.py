...
def remove_na(row):
    for num in xrange(row.count(nan)): row.remove(nan)
    return row
...
def populate_table(conn, reg_session, session):
    ...
    if reg_session > session:   # Accounts for the carry over field for 200 to 500 level
        ...
        curr_frame['CARRYOVER'] = map(lambda x: str(remove_na(x)), carry_overs)
...
def build_db(conn, session):
    cursor = conn.cursor()
    for reg_session in xrange(session, session + 5):
        ...
        if reg_session == session:  # 100 level
            stmt = 'CREATE TABLE CourseReg{0}00('.format(reg_session - session + 1,) + ','.join(map(lambda x: x + ' INTEGER', header[:-1])) + ',{} TEXT'.format(header[-1]) + ')'
        else:   # 200 - 500 level
            ...
            stmt = 'CREATE TABLE CourseReg{0}00({1} INTEGER, {2} TEXT,'.format(reg_session - session + 1, header[0], header[1]) + ','.join(map(lambda x: x + ' INTEGER', header[2:-1])) + ',{} TEXT'.format(header[-1]) + ')'
        try:
...
paths = glob.glob(os.path.join(os.getcwd(), 'data', '[1-5]*.csv'))    # List of couse reg paths
